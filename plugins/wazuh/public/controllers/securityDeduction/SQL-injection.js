...
export const SQLInjection = compose(
	withReduxProvider,
	withGlobalBreadcrumb([{ text: '' }, { text: '安全推演' }, { text: 'SQL注入' }]),
          ...
          <EuiFlexGroup justifyContent={'spaceBetween'}>
            <EuiFlexItem grow={false}>
              <span style={{ display: 'inline-flex' }}>
                <EuiTitle size="s">
                  <h1>
                    <span>&nbsp;{WAZUH_MODULES['SQLInjection'].title}&nbsp;&nbsp;</span>
                  </h1>
                </EuiTitle>
                <EuiPopover
                  button={
                  ...
                  <div style={{ width: '400px' }}>
                    {WAZUH_MODULES['SQLInjection'].description}
                  </div>
                </EuiPopover>
              </span>
            </EuiFlexItem>
  ...
  render() {
    ...
    return (
      <EuiFlexGroup direction="column">
              ...
              <EuiPage>
                <Deduction ref={deductionRef} deductionUtl="/manager/analysisd/sql_attack" planSaveUrl={isCluster ? '/cluster/sql_attack/' : '/manager/sql_attack/'} type="sql_attack" planOptions={planOptions} configPlan={['1']} noConfigMess={'该方案不需要配置，且会清除已配置的黑名单，确认保存吗？'} />
              </EuiPage>
						</div>
					</div>
				</EuiFlexItem>
