...
export class RowDetails extends Component {
  ...
  getFilterLink = (key, value) => {
    ...
    filter[key] = value;
    return (
      <EuiToolTip position="top" content={`过滤 ${key}:${value}`}>
  ...
  renderRows() {
    const fieldsToShow = ['agent', 'cluster', 'manager', 'rule', 'decoder', 'syscheck', 'full_log', 'location', 'data.win.system.message'];
    var rows: any[] = [];
            ...
            {(this.state.hover === key &&
              <EuiFlexGroup style={{ height: 35 }}>
                {/* <EuiFlexItem grow={false} style={{ marginRight: 0, marginTop: 8 }}>
                  <EuiToolTip position="top" content={`过滤值`}>
                    <EuiButtonIcon
                ...
                <EuiFlexItem grow={false} style={{ marginRight: 0, marginLeft: 0, marginTop: 8 }}>
                  <EuiToolTip position="top" content={`切换列`}>
                    <EuiButtonIcon
    ...
    return (
      <EuiFlexGrid columns={4}>
        <EuiFlexItem key="id" grow={1}>
          ...
          <EuiToolTip position="top" content={`过滤规则ID: ${id}`}>
            <EuiLink
        ...
        <EuiFlexItem key="level" grow={1}>
          <b style={{ paddingBottom: 6 }}>等级</b>
          <EuiToolTip position="top" content={`过滤等级: ${level}`}>
            <EuiLink
        ...
        <EuiFlexItem key="file" grow={1}>
          <b style={{ paddingBottom: 6 }}>文件</b>{file}
        </EuiFlexItem>
        <EuiFlexItem key="path" grow={1}>
          <b style={{ paddingBottom: 6 }}>路径</b>{path}
        </EuiFlexItem>
        <EuiFlexItem key="Groups" grow={1}><b style={{ paddingBottom: 6 }}>组</b>
        ...
        <EuiFlexGroup>
          <EuiFlexItem style={{ padding: 16 }}>
            ...
            {this.state.selectedTabId === 'rule' && this.state.ruleData.total_affected_items === 1 && (
              ...
              (
                <span>加载规则ID: {this.props.item.rule.id} 时出错</span>
              )}
          </EuiFlexItem>
        </EuiFlexGroup>
      </div>
