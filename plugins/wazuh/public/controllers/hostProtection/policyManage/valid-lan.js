...
export class ValidLan extends Component {
  constructor(props) {
    super(props);
    this.state = {
      items: [],
  ...
  async componentDidUpdate(prevProps, prevState) {
    if (this.props.selectView === 'validLan' && prevProps.policyManageType !== this.props.policyManageType) {
      this.initData(this.props.policyManageType);
    }
  }

  async initData(detailsItem) {
    const isDisconnectItems = await WzRequest.apiReq('GET', `/groups/${detailsItem}/files/valid_lan/flag?pretty=true`, {});
    const isDisconnectItem = ((isDisconnectItems || {}).data || {}).data;
    const isDisconnect = isDisconnectItem === '1' ? true : false;

    const result = await WzRequest.apiReq('GET', `/groups/${detailsItem}/files/valid_lan/xml?pretty=true`, { });
  ...
  async setUpdateisDisconnect(e) {
    ...
    if (isDisconnect && items.length === 0) {
      this.showToast(
        'danger',
        '警告',
        '非法网络自动断网操作切换失败: 当前有线网络白名单内无数据，请确保和管理端连接的网络终端MAC在白名单中，否则会造成代理失联',
        3000
      );
      return
    }
		const rawItems = await WzRequest.apiReq('PUT', `/groups/${this.props.policyManageType}/files/valid_lan/flag?pretty=true`, {});
  ...
  async saveList() {
    try {
      ...
      await WzRequest.apiReq('PUT', isCluster ? '/cluster' + `/groups/${detailsItem}/valid_lan` : `/groups/${detailsItem}/valid_lan`, {
        body: raw.toString(),
        origin: 'raw'
      });
    ...
    return (
      <div>
        ...
        <EuiFlexGroup alignItems="center">
          ...
          { isDisconnect && (
            <EuiFlexItem>
              <EuiTextColor color="subdued">请确保和管理端连接的网络终端MAC在白名单中，否则会造成代理失联</EuiTextColor>
            </EuiFlexItem>
          )}
        </EuiFlexGroup>
