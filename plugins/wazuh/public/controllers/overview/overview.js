...
const buildVisTabs = ['systemCommands', 'webAttack', 'bruteForce',
'logToClear', 'reboundShell', 'localAskRight', 'dataTheft', 'blackmailVirus', 'riskPort',
'accountChange', 'accountUnusual', 'accountLogin', 'zombies', 'hiddenProcess', 'riskService', 'virusFound',
'systemCommandVerify'];
...
export class OverviewController {
  ...
  async $onInit() {
    ...
    this.modulesTab = this.commonData.checkModulesTabLocation();
    this.logModules = this.commonData.checkLogModulesLocation();
  ...
  async updateSelectedAgents(agentList) {
    ...
    if (agentList && agentList.length) {
      let tabName = buildVisTabs.indexOf(this.tab) !== -1 ? 'general' : this.tab;
  ...
  async getSummary() {
    try {
      ...
      if (result) {
        ...
        throw new Error('从API获取/agents/summary时出错');
      }
      return;
    } catch (error) {
