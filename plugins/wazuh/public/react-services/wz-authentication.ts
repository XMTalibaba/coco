...
import { updateUserPermissions, updateUserRoles, updateCurrentRole, updateWithUserLogged } from '../redux/actions/appStateActions';
...
export class WzAuthentication{
  ...
  static async refresh(force = false){
    try {
      ...
      const currentUserInfo = await AppState.getCurrentUserInfo();
      ...
      store.dispatch(updateCurrentRole(currentUserInfo.role));
