...
class WzModuleGuide extends Component {
  ...
  renderSteps() {
    ...
    return [
      ...configurationSteps,
      {
        title: !invalidConfiguration ? '编辑ossec.conf' : '配置错误',
        children: (
          <Fragment>
            {!invalidConfiguration ? (
              <Fragment>
                ...
                {this.state.agentTypeSelected === 'manager' ? (
                  <Fragment>
                    <EuiText>完成模块配置后，复制以上XML配置。去到 <EuiCode>策略管理</EuiCode> {'>'} <EuiCode>配置</EuiCode> {'>'} <EuiCode>编辑配置</EuiCode>，粘贴配置，保存并重新启动管理器或节点。</EuiText>
                  </Fragment>
                  ) : this.state.agentOSSelected === 'linux' ? (
                    <EuiText>完成模块配置后，复制以上XML配置。去到Linux代理的 <EuiCode>/var/ossec/etc</EuiCode> 目录，将上述配置放入 <EuiCode>ossec.conf</EuiCode> 文件中并重新启动代理。</EuiText>
                    ) : (
                      <EuiText>完成模块配置后，复制以上XML配置。去到Windows代理的 <EuiCode>C:\Program Files (x86)\ossec-agent</EuiCode> 目录，将上述配置放入 <EuiCode>ossec.conf</EuiCode> 文件中并重新启动代理。</EuiText>
                )}
                <EuiSpacer size='s' />
                <EuiText>以上部分必须位于顶层 <EuiCode>{`<ossec_config>`}</EuiCode> 标签。</EuiText>
              </Fragment>
              ) : (
                <EuiText color='danger'>配置中有错误，请检查有错误的字段。</EuiText>
            )}
          </Fragment>
        ),
        ...
        {!guideOption.elements ? (
                    ...
                    {guideOption.options.map((guideSubOption, key) => {
                      ...
                      <Fragment key={`${guideOption.name}-options-${guideSubOption.name}`}>
                        ...
                        <EuiFlexGroup justifyContent='center'>
                          ...
                          >
                            <EuiButtonEmpty iconType='plusInCircle' onClick={() => this.addElementToStep([...keyID,'options', key,'elements'], guideSubOption, {ignore_repeatable: true})}>添加 {guideSubOption.name}</EuiButtonEmpty>
                          </EuiToolTip>
                          </EuiFlexItem>
                        </EuiFlexGroup>
                      </Fragment>
          ...
          <Fragment>
            ...
            <EuiFlexGroup justifyContent='center'>
              ...
              >
                <EuiButtonEmpty iconType='plusInCircle' onClick={() => this.addElementToStep([...keyID,'elements'], guideOption, {ignore_repeatable: true})}>添加 {guideOption.name}</EuiButtonEmpty>
              </EuiToolTip>
              </EuiFlexItem>
            </EuiFlexGroup>
          </Fragment>
      ...
      case 'select':
        return (
          <EuiSelect
            ...
            disabled={guideOption.field_disabled}
            onChange={(e) => this.setElementProp(keyID, 'value', e.target.value)}
            aria-label={`${guideOption.name}-选择`}
          />
        )
      default:
        return null
                ...
                <EuiTitle size='l'>
                  <h2>
                    <EuiToolTip
                      ...
                      content='返回添加模块数据'
                    >
                      <EuiButtonIcon onClick={() => this.props.close()} iconType='arrowLeft' iconSize='l' aria-label='返回添加模块数据'/>
                    </EuiToolTip>
                    {guide.icon && (
              ...
              <EuiFlexGroup>
                ...
                <EuiFlexItem grow={false}>
                  <EuiFlexGroup justifyContent='flexEnd'>
                    <EuiFlexItem>
                      <EuiButtonEmpty iconType='refresh' onClick={() => this.toggleResetGuideModal()}>重置指南</EuiButtonEmpty>
                    </EuiFlexItem>
                  </EuiFlexGroup>
                </EuiFlexItem>
              </EuiFlexGroup>
