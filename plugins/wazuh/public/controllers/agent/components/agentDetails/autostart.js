...
export class Autostart extends Component {
  constructor(props) {
		super(props);
		this.state = {
      pageIndex: 0,
  ...
  async componentDidMount() {
    this._isMount = true;
    if (this.props.selectView === 'autostart') this.getItems();
  }
  async componentDidUpdate(prevProps, prevState) {
    if ((this.props.selectAgent.id && this.props.selectView === 'autostart')
  ...
  async getItems() {
    try {
      ...
      const rawItems = await WzRequest.apiReq(
        'GET',
        `/syscollector/${selectAgent.id}/autostart?pretty=true`,
  ...
  tableRender() {
    ...
    return (
      <div>
        <EuiFieldSearch
          placeholder={`过滤开机启动项列表`}
          value={search}
          fullWidth={true}
          onChange={e => this.changeSearch(e)}
          aria-label={`过滤开机启动项列表`}
        />
        <EuiSpacer size="m" />
        <EuiBasicTable
          items={listItems}
