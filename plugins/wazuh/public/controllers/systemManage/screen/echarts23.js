...
export class Echarts23 extends Component {
  constructor(props) {
    super(props);
  ...
  async toSearch() {
    ...
    try {
      ...
      const res = await WzRequest.apiReq('GET',`/manager/eventlog_analyze?name=latest_alerts`, {});
		alerts = (
        ((res || {}).data || {}).data || {}
      ).affected_items[0].latest_inputsource_alerts;
    ...
    return (
      <div style={{ width: '100%', height: '100%' }}>
        <div style={{ display: 'flex', height: '20px', lineHeight: '20px', backgroundColor: '#224B77', marginBottom: '10px' }}>
          <p style={{ flex: '1', paddingLeft: '8px' }}>服务器</p>
          <p style={{ flex: '1', paddingLeft: '8px' }}>端口号</p>
              ...
              {alerts.map((item, index) => (
                <div className="swiper-slide" key={`echartsBox22_${index}`}>
                  <EuiFlexGroup alignItems="center">
                    <EuiFlexItem grow={1} style={{ width: 150 }}><p style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'}}>{item.host}</p></EuiFlexItem>
                    <EuiFlexItem grow={1} style={{ width: 150 }}><p style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'}}>{item.port}</p></EuiFlexItem>
                    <EuiFlexItem grow={1} style={{ width: 150}}><p style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'}} title={item.message}>{item.message}</p></EuiFlexItem>
