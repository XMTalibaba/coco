    ...
    const data = [
      {
        ...
        description: ( <EuiToolTip
          position="top"
          content={"在新标签页打开 " + currentTechnique + " 详情"}>
        ...
        <EuiSpacer size='s' />
          <EuiAccordion
            ...
            buttonContent={
              <EuiTitle size="s">
                <h3>
                  近24小时内事件{this.props.view && this.props.view !== 'events' && (
          ...
          <EuiFlexGroup className="flyout-row">
            <EuiFlexItem>
              <Discover shareFilterManager={this.filterManager} initialColumns={["icon", "timestamp", 'rule.mitre.id', 'rule.mitre.tactic', 'rule.level', 'rule.id', 'rule.description']} implicitFilters={implicitFilters} initialFilters={[]} updateTotalHits={(total) => this.updateTotalHits(total)}/>
