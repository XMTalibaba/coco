...
export const createGaugeVisTypeDefinition = (deps: VisTypeVislibDependencies) => ({
  name: 'gauge',
  title: i18n.translate('visTypeVislib.gauge.gaugeTitle', { defaultMessage: '仪表盘' }),
  icon: 'visGauge',
  description: i18n.translate('visTypeVislib.gauge.gaugeDescription', {
    defaultMessage:
      "仪表盘图指示指标的状态。用于显示指标值与参考阈值的相关程度.",
  }),
  visConfig: {
    defaults: {
      type: 'gauge',
    ...
    schemas: new Schemas([
      {
        ...
        name: 'metric',
        title: i18n.translate('visTypeVislib.gauge.metricTitle', { defaultMessage: '指标' }),
        min: 1,
        aggFilter: [
          '!std_dev',
          '!geo_centroid',
