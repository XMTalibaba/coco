...
import { AppState as WazuhAppState } from '../../../../react-services/app-state';
...
export function DiscoverLegacy({
  ...
  const getDepartmentInfo = async () => {
    const currentUserInfo = await WazuhAppState.getCurrentUserInfo()
    const departmentAgents = await WazuhAppState.getDepartmentAgents()
    setCurrentUserInfo(currentUserInfo)
    setDepartmentAgents(departmentAgents)
  };
  
  const url = window.location.href;
  new RegExp(".*\\b"+ "tab" +"\\b(\\s*=([^&]+)).*", "gi").test(url);
  const tab = RegExp.$2;

  // 查询 agent.id 不等于 000 的告警
  const baseNotFilter = buildFilter(
    indexPattern,
  ...
  // 默认搜索条件(风险检测和安全日志统一适用)
  let formattedFilter = [];
  if (WAZUH_MODULES_SEARCH_INIT[tab]) {
    const searchInitArr = WAZUH_MODULES_SEARCH_INIT[tab];
    searchInitArr.length > 0 && searchInitArr.forEach(searchInit => {
      if (searchInit.acton === 'dis') {
        if (searchInit.type === 'phrasesFilter') { // 默认属于类型筛选
          formattedFilter.push(buildPhrasesFilter({ name: searchInit.params.name, type: "string" }, searchInit.params.params, indexPattern));
        }
        else if (searchInit.type === 'phraseFilter') { // 默认是类型筛选
          formattedFilter.push(buildPhraseFilter({ name: searchInit.params.name, type: "string" }, searchInit.params.params, indexPattern));
        }
        else if (searchInit.type === 'customFilter') { // 查询DSL语句
          formattedFilter.push(buildCustomFilter(indexPattern.title, searchInit.body, false, false, null, FilterStateStore.APP_STATE));
        }
      }
    })
  }
  if (currentUserInfo && currentUserInfo.department && departmentAgents) {
    if (departmentAgents.length > 0) {
      formattedFilter.push(buildPhrasesFilter({ name: 'agent.id', type: "string" }, departmentAgents.map(k => k.id), indexPattern));
    }
    else {
      formattedFilter.push(buildPhrasesFilter({ name: 'agent.id', type: "string" }, [''], indexPattern));
    }
  }
  
  filterManager.addFilters([baseNotFilter, ...(formattedFilter.length > 0 ? formattedFilter : [])]);
                          ...
                          {rows.length === opts.sampleSize && (
                            ...
                            >
                              <FormattedMessage
                                ...
                                defaultMessage="这些是与您的搜索相匹配的第一批{sampleSize}文件，请细化您的搜索以查看其他文件。"
