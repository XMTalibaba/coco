...
export class AgentsController {
  ...
  onClickUpgrade() {
    try {
      WzRequest.apiReq('PUT', `/agents/${this.$scope.agent.id}/upgrade`, {})
        .then(() => {
          this.showToast('success', '代理正在升级。', '', 5000);
        })
        .catch(() => {
          this.showToast('warning', '该代理已经升级。', '', 5000);
        });
    } catch (error) {
      console.log(error);
    }
  ...
  onClickRestart() {
    try {
      WzRequest.apiReq('PUT', `/agents/${this.$scope.agent.id}/restart`, {})
        .then(() => {
          this.showToast('success', '代理重启。', '', 5000);
        })
        .catch(() => {
          this.showToast('warning', '代理重启错误。', '', 5000);
        });
    } catch (error) {
      console.log(error);
    }
  ...
  async launchRootcheckScan() {
    try {
      ...
      ErrorHandler.info(
        `在 ${this.$scope.agent.id} 代理上成功启动策略监视扫描`);
    } catch (error) {
      ErrorHandler.handle(error);
    }
    return;
  ...
  async launchSyscheckScan() {
    try {
      ...
      ErrorHandler.info(
        `在 ${this.$scope.agent.id} 代理上成功启动FIM扫描`,
        ''
      );
    } catch (error) {
      ErrorHandler.handle(error);
