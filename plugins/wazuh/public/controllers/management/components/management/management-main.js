...
class WzManagementMain extends Component {
  ...
  isClusterOrManager = async () => {
    try {
      ...
      console.warn(`试图获取集群状态时出错`, error);
      this.props.updateClusterStatus({
        status: false,
        contextConfigServer: 'manager',
      });
...
export default compose(withUserAuthorizationPrompt(null, AppState.getPagePermission))(connect(mapStateToProps, mapDispatchToProps)(WzManagementMain))
