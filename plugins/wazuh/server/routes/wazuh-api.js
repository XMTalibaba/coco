...
function WazuhApiRoutes(router) {
  ...
  router.post({
    ...
    validate: {
      body: _configSchema.schema.object({
        ...
        csvKey:  _configSchema.schema.any({
          defaultValue: []
        }),
        csvKeyContrast: _configSchema.schema.any({
          defaultValue: {}
        })
      })
    }
  ...
  router.get({
    ...
    validate: false,
    options: {
      authRequired: false // 不需要登录就可调用
    }
  }, async (context, request, response) => ctrl.getPlatformInfo(context, request, response));

  router.get({
    path: '/api/getPagePermission',
    validate: false,
  }, async (context, request, response) => ctrl.getPagePermission(context, request, response));
}
