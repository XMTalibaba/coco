...
export function createEuiListItem({
  ...
  const { href, id, title, disabled, tooltip } = link;
  const ZH_TITLE: Record<string, string> = {
    wazuh: '系统控制',
    opendistro_security: '用户管理',
    developerExamples: '开发示例',
    kibanaOverview: '视图',
    discover: '日志管理',
    dashboards: '仪表',
    visualize: '统计',
  };
  const ICON_LIST: Record<string, string> = {
    wazuh: 'visArea',
    opendistro_security: 'inspect',
  };

  return {
    label: tooltip ?? ZH_TITLE[id] ?? title,
    href,
