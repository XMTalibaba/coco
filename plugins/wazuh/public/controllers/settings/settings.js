...
export class SettingsController {
  ...
  async $onInit() {
    try {
      // const breadcrumb = [{ text: '' }, { text: '系统管理' }];
  ...
  async setDefault(item) {
    try {
      ...
      ErrorHandler.info(`ID为${idApi}的API被设置为默认值`);
  ...
  async getSettings() {
    try {
      ...
      if (!this.indexPatterns.length) {
        this.wzMisc.setBlankScr('抱歉，没有找到有效的索引模式');
      ...
      ErrorHandler.handle('获取API条目时出错', 'Settings');
    }
  ...
  async checkManager(item, isIndex, silent = false) {
    try {
      ...
      !silent && ErrorHandler.info('连接成功', 'Settings');
      this.$scope.$applyAsync();
      return;
    } catch (error) {
      this.load = false;
  ...
  async checkForNewApis() {
    try {
      ...
      if (notRecheable) {
        ...
        throw {
          message: `有些API条目无法访问。您仍然可以使用应用程序，但请查看主机配置。`,
          type: 'warning',
          closedEnabled: true
        };
      }
