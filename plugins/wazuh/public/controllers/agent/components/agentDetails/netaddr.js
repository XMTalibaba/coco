...
export class Netaddr extends Component {
  constructor(props) {
		super(props);
		this.state = {
      pageIndex: 0,
  ...
  async componentDidMount() {
    this._isMount = true;
    if (this.props.selectView === 'netaddr') this.getItems();
  }
  async componentDidUpdate(prevProps, prevState) {
    if ((this.props.selectAgent.id && this.props.selectView === 'netaddr')
  ...
  async getItems() {
    try {
      ...
      const rawItems = await WzRequest.apiReq(
        'GET',
        `/syscollector/${selectAgent.id}/netaddr`,
  ...
  getColumns() {
    ...
    columns = netaddrColumns.map(item => {
  ...
  async downloadCsv () {
    ...
    const csvKey = ['address', 'netmask', 'proto', 'iface', 'broadcast', 'agent_id', 'agent_ip', 'agent_name'];
    await AppState.downloadCsv(
      `/syscollector/${selectAgent.id}/netaddr`,
      `${selectAgent.name}-网络设置.csv`,
