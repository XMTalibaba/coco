...
export async function getElasticAlerts(indexPattern, filterParams:IFilterParams, aggs:any=null, kargs={}, departmentInfo={}) {
  ...
  if (departmentInfo.currentUserInfo && departmentInfo.currentUserInfo.department) {
    let should = []
    if (departmentInfo.departmentAgents.length > 0) {
      departmentInfo.departmentAgents.forEach(k => {
        should.push({
