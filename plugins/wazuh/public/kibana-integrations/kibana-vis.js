...
class KibanaVis extends Component {
  ...
  constructor(props) {
    ...
    // 自定义告警页，用general(安全事件页)相同的图表
    let newTab = this.props.tab;
    if (WAZUH_MODULES_SEARCH_INIT[this.props.tab]) {
      ...
      searchInitArr.length > 0 && searchInitArr.forEach(searchInit => {
        if (searchInit.acton === 'vis') { // 默认初始查询sql语句
          newTab = 'general'
        }
      })
    }
  ...
  componentDidUpdate(prevProps) {
    if (this.props.isAgent !== prevProps.isAgent && prevProps.isAgent && this.props.isAgent) { // 切换代理
      this.visualization = null;
      this.renderInProgress = false;
      this.rendered = false;
    }
  ...
  myRender = async (raw) => {
    ...
    if (WAZUH_MODULES_SEARCH_INIT[this.props.tab]) {
      const searchInitArr = WAZUH_MODULES_SEARCH_INIT[this.props.tab];
      searchInitArr.length > 0 && searchInitArr.forEach(searchInit => {
        if (searchInit.acton === 'vis' && searchInit.type === 'query') { // 默认初始查询sql语句
          getDataPlugin().query.queryString.setQuery({ query: searchInit.params.sql, language: "kuery" });
        }
      })
    }
    ...
    try {
      ...
      let query = !isAgentStatus ? discoverList[0] : {};

      if (this.tab === 'workbench') { // 控制台页
        filters = discoverList[0][this.visID]
        query = {query: '', language: 'kuery', timestamp: new Date().getTime()}
      }
        ...
        <span>
          ...
          >
            <EuiFlexGroup style={{ placeItems: 'center' }}>
              ...
              <EuiFlexItem grow={false}>刷新索引模式。</EuiFlexItem>
