    ...
    if (exportDetails && exportDetails.missingReferences.length > 0) {
      notifications.toasts.addWarning({
        title: i18n.translate(
          ...
          {
            defaultMessage:
              '您的文件正在后台下载。找不到某些相关对象。有关缺失对象列表，请查看导出文件的最后一行.',
          }
        ),
      });
    } else {
          ...
          <EuiModalHeader>
            <EuiModalHeaderTitle>
              <FormattedMessage
                ...
                defaultMessage="导出 {filteredItemCount, plural, one{# 个对象} other {# 个对象}}"
                values={{
                  filteredItemCount,
                }}
              />
