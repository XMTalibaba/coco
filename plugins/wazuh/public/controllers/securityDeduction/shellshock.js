...
export const Shellshock = compose(
	withReduxProvider,
	withGlobalBreadcrumb([{ text: '' }, { text: '安全推演' }, { text: 'Shellshock' }]),
	withUserAuthorizationPrompt(null, AppState.getPagePermission)
)(class Shellshock extends Component {
  _isMount = false;
  constructor(props) {
		super(props);
    this.myChart = null;
          ...
          <EuiFlexGroup justifyContent={'spaceBetween'}>
            <EuiFlexItem grow={false}>
              <span style={{ display: 'inline-flex' }}>
                <EuiTitle size="s">
                  <h1>
                    <span>&nbsp;{WAZUH_MODULES['shellshock'].title}&nbsp;&nbsp;</span>
                  </h1>
                </EuiTitle>
                <EuiPopover
                  button={
                  ...
                  <div style={{ width: '400px' }}>
                    {WAZUH_MODULES['shellshock'].description}
                  </div>
                </EuiPopover>
              </span>
            </EuiFlexItem>
  ...
  render() {
    ...
    return (
      <EuiFlexGroup direction="column">
              ...
              <EuiPage>
                <Deduction ref={deductionRef} deductionUtl="/manager/analysisd/shellshock" planSaveUrl={isCluster ? '/cluster/shellshock/' : '/manager/shellshock/'} type="shellshock" planOptions={planOptions} configPlan={['1']} noConfigMess={'该方案不需要配置，且会清除已配置的黑名单，确认保存吗？'} />
              </EuiPage>
						</div>
					</div>
				</EuiFlexItem>
