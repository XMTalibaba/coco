    ...
    if (body === undefined) {
      ...
      throw new Error(
        i18n.translate('visTypeVega.esQueryParser.urlBodyValueTypeErrorMessage', {
          defaultMessage: '{configName} 必须为对象',
    ...
    if (legacyContext !== undefined) {
      if (body.query !== undefined) {
        throw new Error(
          i18n.translate(
            ...
            {
              defaultMessage:
                '{dataUrlParam} 不得同时具有旧的 {legacyContext} 和 {bodyQueryConfigName} 值',
              values: {
        ...
        throw new Error(
          i18n.translate(
            ...
            {
              defaultMessage:
                '{dataUrlParam} 不得同时具有 {legacyContext} 与 {context} 或 {timefield}',
              values: {
                legacyContext: `"${LEGACY_CONTEXT}"`,
                context: `"${CONTEXT}"`,
                timefield: `"${TIMEFIELD}"`,
        ...
        throw new Error(
          i18n.translate('visTypeVega.esQueryParser.legacyContextCanBeTrueErrorMessage', {
            defaultMessage:
              '旧 {legacyContext} 可以为 {trueValue}（忽略时间范围选取器），也可以为时间字段的名称，例如 {timestampParam}',
            values: {
              legacyContext: `"${LEGACY_CONTEXT}"`,
              trueValue: 'true',
      ...
      this._onWarning(
        i18n.translate('visTypeVega.esQueryParser.legacyUrlShouldChangeToWarningMessage', {
          defaultMessage: '旧 {urlParam}：{legacyUrl} 应更改为 {result}',
          values: {
      ...
      if (usesContext) {
        throw new Error(
          i18n.translate(
            ...
            {
              defaultMessage:
                '设置了 {queryParam} 时，不得使用 {urlContext} 和 {timefield}',
              values: {
            ...
            if (size === true) {
              ...
              throw new Error(
                i18n.translate('visTypeVega.esQueryParser.autointervalValueTypeErrorMessage', {
                  defaultMessage: '{autointerval} 必须为 {trueValue} 或数字',
                  values: {
      ...
      if (typeof shift !== 'number') {
        throw new Error(
          i18n.translate('visTypeVega.esQueryParser.shiftMustValueTypeErrorMessage', {
            defaultMessage: '{shiftParam} 必须为数值',
            values: {
              shiftParam: '"shift"',
            },
          })
