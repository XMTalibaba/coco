...
const VALID_LENGTH_HELP_TEXT = (resourceType: string) => {
  return `${resourceType === 'user' ? '用户' : resourceType}名长度为${MIN_NUMBER_OF_CHARS_IN_RESOURCE_NAME}~${MAX_NUMBER_OF_CHARS_IN_RESOURCE_NAME}个字符。`;
};

const VALID_CHARACTERS_HELP_TEXT =
  '有效字符为A-Z、A-Z、0-9、(_)下划线、(-)连字符和unicode字符。';
...
export function validateResourceName(resourceType: string, resourceName: string): string[] {
  ...
  if (!isValidResourceName(resourceName)) {
    errors.push(`在${resourceType === 'user' ? '用户' : resourceType}名中发现无效字符。${VALID_CHARACTERS_HELP_TEXT}`);
  }
  return errors;
}
