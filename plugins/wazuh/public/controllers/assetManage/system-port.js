...
export const SystemPort = compose(
	withReduxProvider,
	withGlobalBreadcrumb([{ text: '' }, { text: '资产管理' }, { text: '系统端口' }]),
	withUserAuthorizationPrompt(null, AppState.getPagePermission)
)(class SystemPort extends Component {
  constructor(props) {
		super(props);
		this.state = {
      ...
      listProps: { // 列表页组件传参
        'agent': {
          ...
          searchUrl: '/agents/summary/ports?', // 查询路径
          detailUrl: 'ports', // 详情接口路径，拼接到后边
        },
        'ports': {
          paramsOptions: [
            // {
          ...
          searchUrl: '/agents/summary/ports?name=all',
        }
      },
      detailProps: { // 详情页组件传参
        'agent': {
          columns: [ // 表格列
            { field: 'local.ip', name: '本地IP' },
            { field: 'local.port', name: '本地端口' },
            { field: 'agent_id', name: '代理ID' },
            { field: 'agent_name', name: '代理名称' },
            { field: 'agent_ip', name: '代理IP' },
            { field: 'state', name: '状态' },
            { field: 'protocol', name: '协议' },
          ],
          searchUrl: '', // 查询路径
          params: {}
        },
          ...
          <EuiFlexGroup justifyContent={'spaceBetween'}>
            <EuiFlexItem grow={false}>
              <span style={{ display: 'inline-flex' }}>
                <EuiTitle size="s">
                  <h1>
                    <span>&nbsp;{WAZUH_MODULES['systemPort'].title}&nbsp;&nbsp;</span>
                  </h1>
                </EuiTitle>
                <EuiPopover
                  button={
                  ...
                  <div style={{ width: '400px' }}>
                    {WAZUH_MODULES['systemPort'].description}
                  </div>
                </EuiPopover>
              </span>
            </EuiFlexItem>
